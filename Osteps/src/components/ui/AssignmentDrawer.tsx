...
const AssignmentDrawer: React.FC<AssignmentDrawerProps> = ({
  ...
  const normalizeWholeMark = (value: unknown, maxMark?: number) => {
    ...
    const wholeMark = Math.trunc(parsed);
    if (!Number.isFinite(maxMark)) return Math.max(0, wholeMark);

    return Math.max(0, Math.min(wholeMark, maxMark as number));
  };
 ...
 const handleSubmit = async (values: any) => {
  ...
  try {
    ...
    if (values.selfAssessment !== undefined) {
      const normalizedSelfAssessment = normalizeWholeMark(
        values.selfAssessment,
        selectedTask?.allocated_marks
      );

      if (normalizedSelfAssessment === null) {
        messageApi.error("Self-assessment must be a whole number.");
        return;
      }

      formData.append(
        "self_assessment_mark",
        normalizedSelfAssessment.toString()
      );
    }
                    ...
                    <p className="text-gray-600">
                      {normalizeWholeMark(
                        selectedTask.selfAssessment ??
                          selectedTask.self_assessment_marks
                      ) ?? 0}
                    </p>
                  </div>
                )}
                        ...
                        { required: true, message: "Please rate your work" },
                        {
                          validator: (_, value) => {
                            ...
                            return Promise.reject(
                              new Error("Please enter a whole number")
                            );
                          },
                        },
                      ]}
                      ...
                      <div>
                        ...
                        {inputError && (
                          <p className="text-red-500 text-xs mt-1">
                            Rating must be a whole number between 0 and{" "}
                            {selectedTask.allocated_marks}
                          </p>
                        )}
                      </div>
