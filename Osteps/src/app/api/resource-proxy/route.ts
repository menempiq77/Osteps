...
const buildClientRuntimeShim = (
  ...
  return [
    ...
    "    if (document.body.dataset.ostepsPreviewFallback === 'true') return;",
    "    const pageText = document.body.innerText || '';",
    "    if (!pageText.includes(KNOWN_APP_ERROR_TEXT)) return;",
    "",
    ...
    "      '<a href=\"' + TARGET_URL.replace(/&/g, '&amp;').replace(/\"/g, '&quot;') + '\" target=\"_blank\" rel=\"noreferrer\" style=\"display:inline-flex;align-items:center;justify-content:center;padding:12px 18px;border-radius:999px;background:#0f766e;color:#ffffff;text-decoration:none;font-weight:600;\">Open Website</a>' +",
    "      '</div>' +",
    "      '</div>';",
    "  };",
    "",
    "  const schedulePreviewFallbackCheck = () => {",
    "    window.setTimeout(renderPreviewFallback, 120);",
    "  };",
    "",
    ...
    "    window.setTimeout(renderPreviewFallback, 1500);",
    "  });",
    "",
