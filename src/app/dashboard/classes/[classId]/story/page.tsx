...
export default function ClassStoryPage() {
  ...
  const [composerAttachmentType, setComposerAttachmentType] = useState<"file" | "link" | undefined>(
    undefined
  );
  const [openComments, setOpenComments] = useState<Record<string, boolean>>({});
  const [commentDrafts, setCommentDrafts] = useState<Record<string, string>>({});
  const [editingPostId, setEditingPostId] = useState<string | null>(null);
  const [editingComment, setEditingComment] = useState<{ itemId: string; commentId: string } | null>(
    null
  );
  ...
  const upsertPostMutation = useMutation({
    ...
    onError: () => {
      messageApi.error(editingPostId ? "Could not update post" : "Could not publish post");
    },
  });
  ...
  return (
    <div className="min-h-screen bg-[#f5f7fd] p-3 md:p-6">
      ...
      <input ref={fileInputRef} type="file" className="hidden" onChange={handleFilePick} accept="*/*" />
          ...
          <div className="flex items-center gap-4">
            <div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-full bg-slate-200 text-lg font-semibold text-slate-700 shadow-sm">
              {currentUser?.profile_path ? (
                ...
                <img src={currentUser.profile_path} alt={currentUser.name || "Profile"} className="h-full w-full object-cover" />
              ) : (
          ...
          <div className="space-y-5">
            {filteredFeed.map((item) => {
              ...
              const canRenderWebsitePreview =
                inlineUrl && !youtubeVideoId && !canRenderDirectVideo && !isImageAttachment(inlineUrl);
              const hideAttachmentCard = item.attachmentType === "link" && Boolean(inlineUrl);
                ...
                >
                  <div className="px-5 py-5 md:px-6 md:py-6">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex min-w-0 items-start gap-3">
                        <div className="relative mt-1">
                          <div className={`absolute -left-2 top-2 h-12 w-1 rounded-full ${typeAccent[item.type]}`} />
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {canEditPost(item) && (
                          <div className="flex items-center gap-1">
                            ...
                            >
                              <Button type="text" danger icon={<DeleteOutlined />} className="!rounded-full" />
                            </Popconfirm>
                          </div>
                        )}
                      </div>
                    ...
                    <div className="mt-5 space-y-4 pl-[68px]">
                      <div className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold capitalize ${typeChip[item.type]}`}>
                        {item.type}
                      </div>
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-white text-[#5a66a0] shadow-sm">
                                {item.attachmentType === "file" ? <UploadOutlined /> : <LinkOutlined />}
                              </div>
                              <div className="min-w-0">
                                ...
                                <div className="truncate text-sm text-slate-500">
                                  {item.attachmentType === "file" ? "Attached file" : item.attachmentUrl}
                                </div>
                              </div>
                            </div>
                          </a>
                    ...
                    {openComments[item.id] && (
                      <div className="mt-4 space-y-3 border-t border-[#eef2f8] pt-4">
                        {comments.map((comment) => (
                          <div key={comment.id} className="flex items-start justify-between gap-3 rounded-2xl bg-[#f7f9fd] px-4 py-3">
                            <div>
                              ...
                              <div className="flex items-center gap-1">
                                <Button
                                  ...
                                  onClick={() => {
                                    ...
                                    setCommentDrafts((prev) => ({ ...prev, [item.id]: comment.body }));
                                  }}
                                />
                                <Popconfirm
          ...
          <div className="space-y-5 px-6 py-5">
            ...
            >
              <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-white text-xl text-[#5a66a0] shadow-sm">
                {composerAttachmentType === "file" ? <UploadOutlined /> : <PictureOutlined />}
              </div>
              <div className="mt-3 text-base font-semibold text-slate-800">Add Photos/Videos</div>
              <div className="text-sm text-slate-500">Click to upload or drag and drop any file</div>
            ...
            <Input
              ...
              onChange={(event) => {
                ...
                setComposerAttachmentType(event.target.value.trim() ? "link" : undefined);
              }}
