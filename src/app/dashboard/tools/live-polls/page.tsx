...
const PollResultsChart = dynamic(
  () => import("./_components/PollResultsChart"),
  {
    ssr: false,
    loading: () => <div className="min-h-[180px] animate-pulse rounded-lg bg-gray-50" />,
  }
);
...
export default function LivePollsPage() {
    ...
    return (
      <div className="flex min-h-[80vh] items-center justify-center p-4">
        <div className="w-full max-w-lg rounded-2xl border border-gray-200 bg-white p-8 shadow-sm">
          <h1 className="text-xl font-bold text-center mb-1 text-gray-900">{joinedPoll.title}</h1>
          ...
          {allDone ? (
            ...
            <div>
              ...
              <h2 className="text-lg font-semibold mb-4 text-gray-900">{q.question_text}</h2>
              {submitted.has(q.id) ? (
                ...
                <>
                  {q.type === "multiple_choice" && q.options ? (
                      ...
                      {q.options.map((opt, i) => (
                        <button
                          ...
                          className={`flex w-full items-center gap-3 rounded-xl border-2 px-4 py-3 text-left text-sm font-medium transition ${
                            answer === opt
                              ? "border-[#4262FF] bg-[#4262FF]/5 text-gray-900"
                    ...
                    <div className="flex justify-center gap-2 my-4">
                      {[1, 2, 3, 4, 5].map((n) => (
                        <button
                          ...
                          className={`h-12 w-12 rounded-xl text-lg font-bold transition ${
                            answer === String(n)
                    ...
                    <textarea
                      ...
                      className="w-full rounded-xl border-2 border-gray-100 bg-gray-50 p-3 text-sm text-gray-900 placeholder-gray-400 focus:border-[#4262FF] focus:outline-none"
                      rows={3}
                    />
                  )}
                  <button
                    ...
                    className="mt-4 w-full rounded-lg bg-[#4262FF] px-6 py-3 font-semibold text-white hover:bg-[#3451E0] disabled:opacity-50"
                  >
              ...
              <div className="mt-4 flex justify-between">
                ...
                <button
                  ...
                  className="text-sm text-[#4262FF] hover:underline disabled:opacity-30"
                >
                  Skip
                </button>
              </div>
  ...
  // ─── PRESENTER VIEW (Mentimeter-style) ───
          ...
          {currentQ ? (
            <div className="w-full max-w-4xl flex flex-col items-center mt-4">
              ...
              <div className="w-full max-w-3xl flex-1 min-h-0">
                <PollResultsChart question={currentQ} mode="presenter" />
              </div>
            </div>
          ) : (
            <div className="text-center">
              <div className="text-6xl mb-4 text-gray-300">&#128203;</div>
              <p className="text-xl text-gray-400">No questions in this poll</p>
            </div>
          )}
        </div>
        ...
        <div className="mx-auto max-w-4xl">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div className="flex-1">
              <h1 className="text-xl font-bold text-gray-900">{activePoll.title} &mdash; Results</h1>
            ...
            {results.questions.map((q, i) => (
              <div key={q.id} className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className="shrink-0 rounded-full border border-gray-200 px-3 py-1 text-xs text-gray-500">
                    {q.total_responses} response{q.total_responses !== 1 ? "s" : ""}
                  </span>
                </div>
                <PollResultsChart question={q} mode="results" />
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="p-3 md:p-6">
        <div className="mx-auto max-w-3xl">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div className="flex-1">
              <h1 className="text-xl font-bold text-gray-900">{activePoll.title}</h1>
            ...
            <div className="flex gap-2">
              ...
              {activePoll.status === "active" && (
                <>
                  <button onClick={() => openPresent(activePoll.id)} className="rounded-lg bg-[#4262FF] px-4 py-2 text-sm font-medium text-white hover:bg-[#3451E0]">
                    Present
                  </button>
              ...
              {activePoll.status === "closed" && (
                <button onClick={() => openResults(activePoll.id)} className="rounded-lg bg-[#4262FF] px-4 py-2 text-sm font-medium text-white hover:bg-[#3451E0]">
                  View Results
                </button>
              )}
            </div>
          ...
          {activePoll.status === "active" && (
            <div className="mb-6 rounded-2xl border border-blue-100 bg-blue-50/50 p-6">
              <div className="flex items-start gap-6">
                <div className="shrink-0 rounded-xl bg-white p-3 shadow-sm border border-gray-100">
                  <QRCodeSVG value={pollUrl} size={120} bgColor="#ffffff" fgColor="#1a1a2e" />
                </div>
                <div className="flex-1">
                  <h3 className="font-bold text-gray-900 text-lg">Share with participants</h3>
                  <p className="text-sm text-gray-500 mt-1">
                    Anyone can join without an account &mdash; just scan or visit the link
                  </p>
                  <div className="mt-3 flex items-center gap-3">
                    <div className="rounded-xl border border-gray-200 bg-white px-5 py-2.5">
                      <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Join code</p>
                      <p className="text-2xl font-mono font-bold tracking-[0.3em] text-gray-900">{activePoll.join_code}</p>
                    </div>
                  </div>
                  <div className="mt-3 flex gap-2">
                    <input
                      readOnly
                      value={pollUrl}
                      className="flex-1 rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-sm text-gray-700 font-mono"
                      onClick={(e) => (e.target as HTMLInputElement).select()}
                    />
                    <button onClick={handleCopyLink} className="rounded-lg bg-[#4262FF] px-4 py-1.5 text-sm font-medium text-white hover:bg-[#3451E0]">
                      {copiedLink ? "Copied!" : "Copy link"}
                    </button>
                  </div>
                </div>
              ...
              <div key={q.id} className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <p className="font-medium mt-0.5 text-gray-900">{q.question_text}</p>
                    {q.type === "multiple_choice" && q.options && (
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        {q.options.map((opt, j) => (
                          <span key={j} className="rounded-full bg-gray-100 px-3 py-1 text-xs text-gray-600">{opt}</span>
                        ))}
                      </div>
                    )}
                  </div>
                  {activePoll.status === "draft" && (
                    <div className="flex gap-1 ml-2">
                      <button onClick={() => handleEditQuestion(q)} className="rounded p-1 text-gray-400 hover:text-[#4262FF] hover:bg-gray-100" title="Edit">
            ...
            <div className="rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 p-5">
              ...
              <div className="space-y-3">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-900 focus:border-[#4262FF] focus:outline-none"
                />
                {qType === "multiple_choice" && (
                  <div className="space-y-2">
                    {qOptions.map((opt, i) => (
                      <div key={i} className="flex gap-2">
                        <input
                          ...
                          className="flex-1 rounded-lg border border-gray-200 px-3 py-1.5 text-sm text-gray-900 focus:border-[#4262FF] focus:outline-none"
                        />
                        {qOptions.length > 2 && (
                          <button
                    ...
                    <button
                      ...
                      className="text-sm text-[#4262FF] hover:underline"
                    >
                      + Add option
                    </button>
                  </div>
                ...
                <div className="flex gap-2 justify-end">
                  ...
                  <button
                    ...
                    className="rounded-lg bg-[#4262FF] px-6 py-2 text-sm font-medium text-white hover:bg-[#3451E0] disabled:opacity-50"
                  >
      ...
      <div className="p-3 md:p-6">
        <div className="mx-auto max-w-lg">
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
            <h2 className="text-lg font-bold mb-4 text-gray-900">Create New Poll</h2>
            <div className="space-y-4">
              ...
              <button
                ...
                className="w-full rounded-lg bg-[#4262FF] px-6 py-3 font-semibold text-white hover:bg-[#3451E0] disabled:opacity-50"
              >
    ...
    <div className="p-3 md:p-6">
      <div className="mx-auto max-w-4xl">
        <div className="flex items-center justify-between gap-4 mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Live Polls</h1>
          ...
          {canCreate && (
            <button
              ...
              className="rounded-lg bg-[#4262FF] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#3451E0]"
            >
              + New Poll
            </button>
          )}
        ...
        <div className="mb-6 rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <h3 className="font-semibold text-sm mb-2 text-gray-700">Join a Poll</h3>
          <div className="flex gap-2">
            <input
              value={joinCode}
              onChange={(e) => { setJoinCode(e.target.value.toUpperCase()); setJoinError(""); }}
              placeholder="Enter 6-digit code"
              className="flex-1 rounded-lg border border-gray-200 px-3 py-2 text-sm font-mono tracking-widest uppercase text-gray-900 focus:border-[#4262FF] focus:outline-none"
              maxLength={6}
            />
            <button
              onClick={handleJoin}
        ...
        {canCreate && (
          <div>
            <h3 className="font-semibold text-sm text-gray-400 uppercase mb-3">Your Polls</h3>
            ...
            <div className="space-y-3">
              {polls.map((poll) => (
                <div key={poll.id} className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm hover:shadow-md transition">
                  <div className="flex items-start justify-between">
                    <div
                    ...
                    <div className="flex gap-1 ml-2">
                      {poll.status === "active" && (
                        <button
                          ...
                          className="rounded-lg bg-[#4262FF] px-3 py-1.5 text-xs font-medium text-white hover:bg-[#3451E0]"
                          title="Present live"
                        >
                          Present
                        </button>
