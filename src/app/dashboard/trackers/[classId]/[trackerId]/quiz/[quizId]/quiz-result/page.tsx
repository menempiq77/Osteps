...
export default function QuizResultPage() {
        ...
        if (localAnswers.length > 0) {
          ...
          try {
            ...
            if (Array.isArray(details)) {
              setUserAnswers(
                details.map((detail: { question_id: number; submitted_answer: unknown }) => {
                  const value = detail.submitted_answer;
                  const answer =
                    typeof value === "string" ||
                    typeof value === "number" ||
                    ...
                    (Array.isArray(value) && value.every((item) => typeof item === "number"))
                      ? value
                      : String(value ?? "");
                  return { question_id: detail.question_id, answer };
                })
              );
            }
          } catch (fetchError) {
  ...
  const calculateResults = (): QuizResult => {
    ...
    // Auto-graded questions are everything except subjective ones (short answer /
    // paragraph) that still need a teacher. Score reflects auto-graded questions.
