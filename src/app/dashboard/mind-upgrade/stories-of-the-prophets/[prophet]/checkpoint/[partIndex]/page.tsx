...
function CheckpointContent({
  ...
  useEffect(() => {
    if (!previewMode && alreadyPassed && !miniGamePlayed) {
      router.push(`/dashboard/mind-upgrade/stories-of-the-prophets/${slug}/minigame/${partNum}`);
    }
  ...
  const handleValidateOrder = () => {
    ...
    if (!previewMode && !success && nextAttempts >= 3) {
      ...
      setTimeout(() => {
        ...
        router.push(`/dashboard/mind-upgrade/stories-of-the-prophets/${prophetLabel}#part-${stageStart}`);
      }, 2000);
    }
  };
        ...
        <div style={{ maxWidth: "900px", margin: "0 auto" }}>
          <section style={{ background: "white", borderRadius: "18px", padding: "32px", boxShadow: "0 8px 24px rgba(0,0,0,0.06)" }}>
            ...
            {(alreadyPassed || (validated && isCorrect)) && (
              <Link href={`/dashboard/mind-upgrade/stories-of-the-prophets/${prophetLabel}#part-${partNum + 1}`} style={{ display: "inline-block", marginTop: "18px", padding: "12px 18px", borderRadius: "10px", background: "#0ea5e9", color: "white", fontWeight: 800, textDecoration: "none" }}>
      ...
      <main style={{ minHeight: "100vh", background: "linear-gradient(180deg, #f0fdf4 0%, #ecfdf5 100%)", padding: "44px 20px" }}>
        <div style={{ maxWidth: "900px", margin: "0 auto" }}>
          <div style={{ marginBottom: 18 }}>
            <Link href={`/dashboard/mind-upgrade/stories-of-the-prophets/${prophetLabel}#part-${partNum}`} style={{ color: "var(--foreground)", textDecoration: "none", fontWeight: 700, background: "var(--background)", border: "1px solid rgba(0,0,0,0.10)", padding: "10px 14px", borderRadius: "12px", display: "inline-block" }}>
              ← Back to Lesson
            </Link>
          </div>
