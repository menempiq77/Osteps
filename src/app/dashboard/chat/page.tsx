...
export default function ChatPage() {
  ...
  const handleDeleteConversation = async (convId: number) => {
    try {
      ...
      showNotif("Failed to delete conversation");
    }
  };
              ...
              return (
                <div key={conv.id} className="relative">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="flex items-center gap-1 shrink-0 ml-2">
                          ...
                          <span
                            ...
                            className="hidden group-hover:inline-flex items-center justify-center h-5 w-5 rounded hover:bg-red-100 text-gray-400 hover:text-red-500 cursor-pointer"
                            title="Delete chat"
                          >
                  ...
                  {deleteConvId === conv.id && (
                    <div className="absolute inset-0 z-10 flex items-center justify-center bg-white/95 rounded px-2">
                      <span className="text-xs text-gray-700 mr-2">Delete chat?</span>
                      <button onClick={() => handleDeleteConversation(conv.id)} className="text-xs font-medium text-red-600 hover:text-red-800 mr-2">Delete</button>
                      <button onClick={() => setDeleteConvId(null)} className="text-xs font-medium text-gray-500 hover:text-gray-700">Cancel</button>
                    </div>
                  )}
                </div>
              );
