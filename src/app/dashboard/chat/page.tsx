...
export default function ChatPage() {
  ...
  // Poll unread count for notifications
  useEffect(() => {
  ...
  const filteredConversations = useMemo(() => {
    if (!searchConversations.trim()) return conversations;
  ...
  return (
    <div className="flex h-[calc(100vh-66px)] overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
      ...
      <div className="flex w-[320px] shrink-0 flex-col border-r border-gray-200 bg-white">
        ...
        <div className="flex items-center justify-between border-b border-gray-100 px-4 py-3">
          ...
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-500 hover:bg-gray-100 hover:text-[var(--primary,#38C16C)] transition"
            title="New Chat"
          >
        ...
        <div className="px-3 py-2">
          <input
            ...
            className="w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm outline-none focus:border-[var(--primary,#38C16C)] focus:bg-white transition"
          />
        </div>
          ...
          {loading && conversations.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 px-6 text-center">
              <p className="text-sm text-gray-400">
                {searchConversations ? "No matching conversations" : "No conversations yet"}
              </p>
            </div>
          ) : (
            filteredConversations.map((conv) => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-start gap-3 px-4 py-3 text-left transition border-b border-gray-50 ${
                    isActive
      ...
      <div className="flex flex-1 flex-col bg-white">
        {showNewChat ? (
          ...
          <div className="flex flex-1 flex-col">
            <div className="flex items-center gap-2 border-b border-gray-100 px-5 py-3">
              <button
                ...
                className="flex h-7 w-7 items-center justify-center rounded-lg text-gray-500 hover:bg-gray-100 transition"
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M19 12H5M12 19l-7-7 7-7" />
                </svg>
              </button>
              <h3 className="text-sm font-bold text-gray-900">New Chat</h3>
            </div>
            ...
            {selectedUsers.length > 0 && (
              <div className="px-5 pt-3 flex flex-wrap gap-1.5">
                {selectedUsers.map((u) => (
                  ...
                  >
                    {u.name}
                    <button onClick={() => handleSelectUser(u)} className="hover:text-red-500 transition">&times;</button>
                  </span>
                ))}
              </div>
            )}
                  ...
                  return (
                    <button
                      ...
                      className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left transition ${
                        isSelected ? "bg-[var(--theme-soft,#eef9f2)]" : "hover:bg-gray-50"
                      }`}
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <span className={`inline-block rounded px-1.5 py-0.5 text-[10px] font-medium ${getRoleBadgeColor(user.role)}`}>
                          {user.role?.replace(/_/g, " ")}
                        </span>
                      </div>
                      {isSelected && (
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="var(--primary, #38C16C)">
          ...
          <>
            ...
            {/* Messages */}
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3" style={{ background: "#f8fafb" }}>
              {notification && (
                <div className="sticky top-0 z-10 flex justify-center">
                  <span className="rounded-full bg-[var(--primary,#38C16C)] text-white px-3 py-1 text-xs shadow-md">
                    {notification}
                  </span>
                </div>
              )}
              ...
              {messages.map((msg) => {
                ...
                return (
                  <div key={msg.id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                    <div
                      className={`max-w-[60%] rounded-2xl px-4 py-2.5 ${
                        isMe
                      ...
                      {msg.file_url && (
                        msg.file_url.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? (
                          <img
                            src={msg.file_url}
                            alt="Attachment"
                            className="mb-1.5 max-h-48 rounded-lg border border-white/20 object-cover"
                          />
                        ) : (
                          <a
                            href={msg.file_url}
              ...
              {pendingFile && (
                <div className="relative mb-2 inline-flex items-center gap-2 rounded-lg border border-gray-200 px-2 py-1 bg-gray-50">
                  {pendingFile.isImage && pendingFile.preview ? (
                    <img src={pendingFile.preview} alt="Upload preview" className="h-20 w-auto rounded-lg object-cover" />
                  ) : (
          ...
          <div className="flex flex-1 flex-col items-center justify-center text-center px-8">
            <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-[var(--theme-soft,#eef9f2)] mb-4">
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="var(--primary, #38C16C)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />
              </svg>
            </div>
            <h3 className="text-lg font-bold text-gray-900 mb-1">Welcome to Chat</h3>
