...
export default function StudentProfilePage() {
  ...
  const completedTasks = student?.class?.term?.flatMap((term: any) => 
    term.assign_assessments?.flatMap((assignment: any) => 
      assignment.assessment.tasks?.filter((task: any) => 
        task.student_assessment_tasks?.[0]?.status === "completed"
      )
    )
  ).flat().length || 0;

  const totalTasks = student?.class?.term?.flatMap((term: any) => 
    term.assign_assessments?.flatMap((assignment: any) => 
      assignment.assessment.tasks
    )
  ).flat().length || 0;

  const totalPoints = student?.behaviour?.reduce((sum: number, record: any) => 
    sum + (record.behaviour?.points || 0), 0
  ) || 0;

  const positiveBehaviours = student?.behaviour?.filter((record: any) => 
    record.behaviour?.points > 0
  ).length || 0;
     ...
     <Card className="rounded-2xl p-6 !mb-4">
      ...
      {totalTasks > 0 && (
        <Card className="shadow-lg border-0 rounded-2xl !mb-4">
          ...
          <Progress
            ...
            format={percent => `${completedTasks}/${totalTasks} tasks (${percent}%)`}
          />
        </Card>
      )}
      ...
      <Row gutter={[16, 16]}>
          ...
          >
            <div className="space-y-4">
              ...
              <div className="p-4 bg-blue-50 rounded-lg">
                <h4 className="font-semibold text-blue-800 mb-2">School Details</h4>
                <Descriptions column={1} size="small">
                  <Descriptions.Item label="Name">{student?.school?.name}</Descriptions.Item>
                  <Descriptions.Item label="Admin">{student?.school?.school_admin}</Descriptions.Item>
                  <Descriptions.Item label="Contact">{student?.school?.contact}</Descriptions.Item>
                  <Descriptions.Item label="Email">{student?.school?.email}</Descriptions.Item>
                  <Descriptions.Item label="Year Structure">{student?.school?.year_structure}</Descriptions.Item>
                </Descriptions>
              </div>
              ...
              <div className="p-4 bg-green-50 rounded-lg">
                ...
                <Descriptions column={1} size="small">
                  <Descriptions.Item label="Class Name">{student?.class?.class_name}</Descriptions.Item>
                  <Descriptions.Item label="Number of Terms">{student?.class?.number_of_terms}</Descriptions.Item>
                  <Descriptions.Item label="Username">{student?.user_name}</Descriptions.Item>
                  <Descriptions.Item label="Class ID">{student?.class_id}</Descriptions.Item>
                </Descriptions>
              </div>
            </div>
          </Card>
        ...
        <Col xs={24} lg={12}>
          ...
          >
            <Timeline>
              {student?.behaviour?.slice(0, 5).map((record: any, index: number) => (
                <Timeline.Item
                  key={record.id}
                ...
                >
                  <div className="flex justify-between">
                    ...
                    <Tag color={record.behaviour?.color}>{record.behaviour?.name}</Tag>
                  </div>
                  <div className="text-gray-500 text-sm">{record.date}</div>
                  <div className="text-gray-600 text-sm">Points: {record.behaviour?.points}</div>
                </Timeline.Item>
              ))}
            </Timeline>
          </Card>
          ...
          items={student?.class?.term?.map((term: any) => ({
            ...
            label: (
              <div className="flex justify-between items-center">
                ...
                <Tag color="blue">{term.assign_assessments?.length || 0} Assessments</Tag>
              </div>
            ),
            children: (
              <div className="space-y-4">
                {term.assign_assessments?.map((assignment: any) => (
                  <Card
                    ...
                    title={
                      <div className="flex justify-between items-center">
                        <span className="font-semibold text-lg">{assignment.assessment.name}</span>
                        <Tag 
                          color={
                            assignment.status === "completed" ? "green" : 
                            assignment.status === "in progress" ? "blue" : "orange"
                          }
                        >
                          {assignment.status}
                        </Tag>
                    ...
                    <Table
                      ...
                      columns={[
                        ...
                        },
                        {
                          ...
                          width: 200,
                          render: (_, record: any) => {
                            const submission = record.student_assessment_tasks?.[0];
                            return submission ? (
                              <div className="text-center">
                                <Tag 
                                  color={submission.status === "completed" ? "green" : "orange"}
                                  className="mb-2"
                                >
                                  {submission.status}
                                </Tag>
                                <div className="text-sm text-gray-600">
                                  <div>Score: {submission.teacher_assessment_score || "N/A"}</div>
                              ...
                              <Tag color="red" className="mx-auto">Not Submitted</Tag>
                            );
                          },
                        },
                      ]}
